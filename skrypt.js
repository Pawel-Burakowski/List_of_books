...
class Book 
{
    constructor(title, author) 
    {
        this.title = title;
        this.author = author;
        this.id = Date.now(); // timestamp, do nadania id
    }
}
...
class Ui 
{
    deleteBook(e)
    {
        const bookId = e.target.getAttribute("data-book-id") //pobranie atrybutu klikniętego

        e.target.parentElement.parentElement.remove()       //usunięcie całego wiersza
    ...
    clearForm() 
    {
        ...
        document.getElementById("bookForm").classList.remove("was-validated"); //usuwanie klasy bootstrapa
    }
}
